...
def _mplot_function(f, vmin, vmax, logscale):
    ...
    elif f.function_space().value_size == 0:
        C = f.dat.data_ro
        if logscale:
            return plt.tripcolor(_mesh2triang(mesh), C, vmin=vmin, vmax=vmax, norm=cls.LogNorm() )
        else:
    ...
    elif f.function_space().value_size == 1:
        w0 = f.dat.data_ro
...
def multi1_plot(objs, titles, same_colorbar=True, show_axis='off', logscale=False, vmin=None, vmax=None, cmap=None):
    ...
    if vmin is None and vmax is None and same_colorbar:
        ...
        vmax = -1e30
        for f in objs:
            if isinstance(f, fd.function.Function):
                fmin = f.vector().min()
                fmax = f.vector().max()
                if fmin < vmin:
                    vmin = fmin
...
def animate(Vh, state, same_colorbar=True, colorbar=True,
    ...
    def my_animate(i):
        ...
        obj = fd.Function(Vh, state.data[i])
...
def coarsen_v(fun, nx = 16, ny = 16):
    ...
    mesh = fd.Mesh("ad_20.xml")
    V_H = fd.VectorFunctionSpace(mesh, "CG", 1)
    fd.parameters['allow_extrapolation'] = True
    fun_H =  fd.interpolate(fun, V_H)
    fd.parameters['allow_extrapolation'] = False
    return fun_H
